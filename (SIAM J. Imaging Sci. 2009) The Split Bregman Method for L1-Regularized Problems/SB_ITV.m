function u = SB_ITV(f,mu)
% Split Bregman Isotropic Total Variation Denoising
%
% u = arg min_u mu/2*||u-f||_2^2 + ITV(u)
%
% Refs:
...
while err > tol
    ...
    % update s^k, equation (4.6)
    s = sqrt((nablaX*u + bX).^2 + (nablaY*u + bY).^2);
    
    % update d_x^{k+1}, equation (4.4)
    dX = max(s-1/lambda,0) .* (nablaX*u + bX) ./ s;

    % update d_y^{k+1}, equation (4.5)
    dY = max(s-1/lambda,0) .* (nablaY*u + bY) ./ s;      
